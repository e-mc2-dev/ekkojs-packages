...
{
    private static readonly ConcurrentDictionary<int, PgHandle> _handles = new();
    private static int _nextId = 0;

    private class PgHandle
    {
        public NpgsqlConnection Connection { get; }
        public NpgsqlTransaction? Transaction { get; set; }
        public PgHandle(NpgsqlConnection conn) { Connection = conn; }
    }

    [UnmanagedCallersOnly(EntryPoint = "pg_connect")]
    ...
    [UnmanagedCallersOnly(EntryPoint = "pg_query")]
    public static unsafe IntPtr Query(int handle, IntPtr sqlPtr, int sqlLen, IntPtr paramsPtr, int paramsLen)
    {
        try
        {
            ...
            using var cmd = h.Transaction != null
                ? new NpgsqlCommand(sql, h.Connection, h.Transaction)
                : new NpgsqlCommand(sql, h.Connection);
    ...
    [UnmanagedCallersOnly(EntryPoint = "pg_exec")]
    ...
    [UnmanagedCallersOnly(EntryPoint = "pg_begin")]
    ...
    [UnmanagedCallersOnly(EntryPoint = "pg_commit")]
    ...
    [UnmanagedCallersOnly(EntryPoint = "pg_rollback")]
    ...
    [UnmanagedCallersOnly(EntryPoint = "pg_close")]
    ...
    [UnmanagedCallersOnly(EntryPoint = "pg_free")]
    public static void Free(IntPtr ptr)
    {
        if (ptr != IntPtr.Zero) Marshal.FreeCoTaskMem(ptr);
    }

    private static void BindParams(NpgsqlCommand cmd, string json)
    {
    ...
    private static string SerializeReader(NpgsqlDataReader reader)
    {
        var sb = new StringBuilder();
        sb.Append("{\"columns\":[");
        ...
        while (reader.Read())
        {
            ...
            for (int i = 0; i < reader.FieldCount; i++)
            {
                ...
                switch (val)
                {
                    ...
                    case DateTime dt: sb.Append(new DateTimeOffset(dt).ToUnixTimeSeconds()); break;
