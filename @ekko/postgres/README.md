...
Talk to PostgreSQL from your EkkoJS app. Open a connection, run queries and transactions, and read
results back as plain objects. If you'd rather work with models than write SQL, it plugs into `ekko:orm`.

## Install

```bash
ekko add @ekko/postgres
```

## Connect and query

```ts
import { PgClient } from "@ekko/postgres";
...
```ts
import { register } from "@ekko/postgres";
register();
```
