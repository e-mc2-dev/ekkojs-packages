...
{
    private static readonly ConcurrentDictionary<int, MysqlHandle> _handles = new();
    private static int _nextId = 0;

    private class MysqlHandle
    {
        public MySqlConnection Connection { get; }
        public MySqlTransaction? Transaction { get; set; }
        public MysqlHandle(MySqlConnection conn) { Connection = conn; }
    }

    [UnmanagedCallersOnly(EntryPoint = "mysql_connect")]
    public static unsafe int Connect(IntPtr connStringPtr, int connStringLen)
    {
        try
        {
            ...
            _handles[id] = new MysqlHandle(conn);
            return id;
        }
        catch
        {
    ...
    [UnmanagedCallersOnly(EntryPoint = "mysql_query")]
    public static unsafe IntPtr Query(int handle, IntPtr sqlPtr, int sqlLen, IntPtr paramsPtr, int paramsLen)
    {
        try
        {
            ...
            using var cmd = h.Transaction != null
                ? new MySqlCommand(sql, h.Connection, h.Transaction)
                : new MySqlCommand(sql, h.Connection);
    ...
    [UnmanagedCallersOnly(EntryPoint = "mysql_exec")]
    ...
    [UnmanagedCallersOnly(EntryPoint = "mysql_begin")]
    ...
    [UnmanagedCallersOnly(EntryPoint = "mysql_commit")]
    ...
    [UnmanagedCallersOnly(EntryPoint = "mysql_rollback")]
    ...
    [UnmanagedCallersOnly(EntryPoint = "mysql_close")]
    ...
    [UnmanagedCallersOnly(EntryPoint = "mysql_free")]
    public static void Free(IntPtr ptr)
    {
        if (ptr != IntPtr.Zero) Marshal.FreeCoTaskMem(ptr);
    }

    private static void BindParams(MySqlCommand cmd, string json)
    {
    ...
    private static string SerializeReader(MySqlDataReader reader)
    {
        var sb = new StringBuilder();
        sb.Append("{\"columns\":[");
        ...
        while (reader.Read())
        {
            ...
            for (int i = 0; i < reader.FieldCount; i++)
            {
                ...
                switch (val)
                {
                    case sbyte sv: sb.Append(sv); break;
                    case byte bv: sb.Append(bv); break;
                    case short sv: sb.Append(sv); break;
                    case ushort uv: sb.Append(uv); break;
                    case int iv: sb.Append(iv); break;
                    case uint uiv: sb.Append(uiv); break;
                    case long lv: sb.Append(lv); break;
                    case ulong ulv: sb.Append(ulv); break;
                    ...
                    case bool bv: sb.Append(bv ? "true" : "false"); break;
