// Native driver test — SQL Server (no ORM, raw MssqlClient)
// Run: ekko run --allow=fs,ffi:unsafe,net,env ekko-lib/mssql/tests/native-v2-mssql.ts
...
const lib = dlopen(`ekko-lib/mssql/native/${platform}/EkkoMssql.${ext}`, {
  ...
  mssql_begin:{args:[types.i32],returns:types.i32},mssql_commit:{args:[types.i32],returns:types.i32},mssql_rollback:{args:[types.i32],returns:types.i32},
  mssql_close:{args:[types.i32],returns:types.void},
});
function buf(s:string){return new TextEncoder().encode(s);}
function query(h:number,sql:string,params:any={}):any{const s=buf(sql),p=buf(JSON.stringify(params));const j=lib.mssql_query(h,s,s.length,p,p.length)as string;if(!j)return{columns:[],rows:[]};const r=JSON.parse(j);if(r.error)throw new Error(r.error);return r;}
function exec(h:number,sql:string,params:any={}):number{const s=buf(sql),p=buf(JSON.stringify(params));return lib.mssql_exec(h,s,s.length,p,p.length)as number;}
function rows(r:any):any[]{return r.rows.map((row:any[])=>{const o:any={};r.columns.forEach((c:string,i:number)=>o[c]=row[i]);return o;});}

console.log("\n=== SQL Server Native Driver Test ===\n");
...
const cs=buf(`Server=${Ekko.env.get("MSSQL_HOST")||"localhost"},${Ekko.env.get("MSSQL_PORT")||"1434"};Database=${Ekko.env.get("MSSQL_DB")||"ekko_test"};User Id=${Ekko.env.get("MSSQL_USER")||"sa"};Password=${Ekko.env.get("MSSQL_PASS")||"Ekko123!"};TrustServerCertificate=True;Pooling=true`);
const h=lib.mssql_connect(cs,cs.length)as number;
assert("connect",h>0);

exec(h,'DROP TABLE IF EXISTS posts');exec(h,'DROP TABLE IF EXISTS tags');exec(h,'DROP TABLE IF EXISTS users');
exec(h,"IF OBJECT_ID(N'users',N'U') IS NULL CREATE TABLE [users] ([id] INT IDENTITY(1,1) PRIMARY KEY,[name] NVARCHAR(255) NOT NULL,[email] NVARCHAR(255) UNIQUE NOT NULL,[age] INT,[active] INT NOT NULL DEFAULT 1)");
exec(h,"IF OBJECT_ID(N'posts',N'U') IS NULL CREATE TABLE [posts] ([id] INT IDENTITY(1,1) PRIMARY KEY,[title] NVARCHAR(255) NOT NULL,[body] NVARCHAR(255),[user_id] INT NOT NULL,[likes] INT NOT NULL DEFAULT 0)");
exec(h,"IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name=N'idx_posts_user' AND object_id=OBJECT_ID(N'posts')) CREATE INDEX idx_posts_user ON posts (user_id)");
exec(h,"IF OBJECT_ID(N'tags',N'U') IS NULL CREATE TABLE [tags] ([id] INT IDENTITY(1,1) PRIMARY KEY,[name] NVARCHAR(255) UNIQUE NOT NULL)");
...
console.log("\n--- SELECT ---");
assertEq("select all",rows(query(h,'SELECT * FROM [users]')).length,5);
assertEq("first",rows(query(h,'SELECT * FROM [users]'))[0].name,"Alice");
assertEq("col",rows(query(h,'SELECT name FROM [users]'))[0].name,"Alice");
assert("one col",Object.keys(rows(query(h,'SELECT name FROM [users]'))[0]).length===1);

console.log("\n--- WHERE ---");
const al=rows(query(h,'SELECT TOP 1 * FROM [users] WHERE name=@p0',{p0:"Alice"}))[0];
...
console.log("\n--- WHERE compound ---");
assertEq("obj",rows(query(h,'SELECT TOP 1 * FROM [users] WHERE name=@p0',{p0:"Bob"}))[0].name,"Bob");
assertEq("multi",rows(query(h,'SELECT TOP 1 * FROM [users] WHERE (name=@p0 AND age=@p1)',{p0:"Alice",p1:30}))[0].email,"alice@test.com");
...
assertEq("top",rows(query(h,'SELECT TOP 2 * FROM [users]')).length,2);
assertEq("offset",rows(query(h,'SELECT * FROM [users] ORDER BY (SELECT NULL) OFFSET 3 ROWS FETCH NEXT 100 ROWS ONLY')).length,2);
assertEq("top+offset",rows(query(h,'SELECT * FROM [users] ORDER BY (SELECT NULL) OFFSET 1 ROWS FETCH NEXT 2 ROWS ONLY')).length,2);
...
console.log("\n--- first/exists ---");
assertEq("first",rows(query(h,'SELECT TOP 1 * FROM [users]'))[0].name,"Alice");
...
assert("firstOrThrow",(()=>{try{const r=rows(query(h,'SELECT TOP 1 * FROM [users] WHERE name=@p0',{p0:"Nobody"}));if(!r.length)throw new Error("no");return false;}catch{return true;}})());

console.log("\n--- UPDATE ---");
exec(h,'UPDATE [users] SET age=@p0 WHERE name=@p1',{p0:31,p1:"Alice"});
assertEq("update age",rows(query(h,'SELECT TOP 1 * FROM [users] WHERE name=@p0',{p0:"Alice"}))[0].age,31);
...
console.log("\n--- TRANSACTIONS ---");
lib.mssql_begin(h);exec(h,"INSERT INTO [tags] (name) VALUES (@p0)",{p0:"testing"});
assertEq("in-tx",rows(query(h,'SELECT COUNT(*) AS c FROM [tags]'))[0].c,3);
lib.mssql_commit(h);assertEq("commit",rows(query(h,'SELECT COUNT(*) AS c FROM [tags]'))[0].c,3);
lib.mssql_begin(h);exec(h,"INSERT INTO [tags] (name) VALUES (@p0)",{p0:"temporary"});
assertEq("pre-rollback",rows(query(h,'SELECT COUNT(*) AS c FROM [tags]'))[0].c,4);
lib.mssql_rollback(h);assertEq("post-rollback",rows(query(h,'SELECT COUNT(*) AS c FROM [tags]'))[0].c,3);
...
assertEq("null first",rows(query(h,'SELECT TOP 1 * FROM [users] WHERE name=@p0',{p0:"NOPE"})).length,0);
